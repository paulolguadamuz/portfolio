...
import { FiUsers, FiZap, FiMail, FiMessageCircle, FiSend, FiLayout, FiCode, FiExternalLink, FiX } from 'react-icons/fi';
import { useLang } from '../i18n/LanguageContext';

const ACCENT = '#53A3E2';
const SURFACE = '#FFFEFE';

const NovaSiteShowcase = forwardRef(function NovaSiteShowcase({ onClose }, ref) {
  const { t } = useLang();
  const contentRef = useRef(null);
  const timelineRef = useRef(null);

  const features = [
    { icon: FiUsers, key: 'carousel' },
    { icon: FiZap, key: 'animations' },
    { icon: FiMail, key: 'contact' },
    { icon: FiSend, key: 'email' },
  ];

  const contributions = [
    { icon: FiLayout, key: 'carousel_impl' },
    { icon: FiZap, key: 'gsap_anims' },
    { icon: FiMail, key: 'contact_form' },
    { icon: FiMessageCircle, key: 'social' },
  ];

  const stack = [
    { label: 'Frontend', key: 'frontend' },
    { label: 'Backend', key: 'backend' },
    { label: t('novasite.stack.integrations_label'), key: 'integrations' },
  ];
  ...
  useImperativeHandle(ref, () => ({
    animateIn() {
      ...
      const heading = content.querySelector('.ns-heading');
      const featureCards = content.querySelectorAll('.ns-feature');
      const contribHeading = content.querySelector('.ns-contrib-heading');
      const contribItems = content.querySelectorAll('.ns-contrib');
      const stackSection = content.querySelector('.ns-stack');
      const ctaRow = content.querySelector('.ns-cta-row');
      const closeBtn = content.querySelector('.ns-close-btn');
    ...
    animateOut() {
      ...
      const allAnimated = content.querySelectorAll(
        '.ns-heading, .ns-feature, .ns-contrib-heading, .ns-contrib, .ns-stack, .ns-cta-row, .ns-close-btn'
      );
  ...
  return (
    <div ref={contentRef} className="relative py-20 lg:py-28">
      ...
      <button
        onClick={onClose}
        className="ns-close-btn absolute top-6 right-6 lg:right-10 z-10 w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110"
        style={{
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-8 relative">
        {/* Section heading */}
        <div className="ns-heading mb-16 lg:mb-20">
          <p
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-20 lg:mb-24">
          {features.map((feat) => {
            ...
            return (
              <div key={feat.key} className="ns-feature novasite-feature-card">
                <div className="novasite-feature-icon">
                ...
                <h4
                  ...
                  style={{ color: SURFACE }}
                >
                  {t(`novasite.features.${feat.key}.title`)}
                </h4>
                <p
                  className="font-body text-sm leading-relaxed"
                  style={{ color: `${SURFACE}88` }}
                >
                  {t(`novasite.features.${feat.key}.desc`)}
                </p>
              </div>
            );
          })}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-20 lg:mb-24">
          {contributions.map((contrib) => {
            ...
            return (
              <div key={contrib.key} className="ns-contrib novasite-contrib-item">
                <div
                ...
                <div>
                  <h5
                    ...
                    style={{ color: SURFACE }}
                  >
                    {t(`novasite.contributions.${contrib.key}.title`)}
                  </h5>
                  <p
                    className="font-body text-xs sm:text-sm leading-relaxed"
                    style={{ color: `${SURFACE}77` }}
                  >
                    {t(`novasite.contributions.${contrib.key}.desc`)}
                  </p>
                </div>
              </div>
            );
        ...
        {/* Tech Stack */}
        <div className="ns-stack novasite-stack-section mb-14">
          <h4
            className="font-display font-bold text-lg sm:text-xl mb-6"
            style={{ color: SURFACE }}
          >
            {t('novasite.stack.title')}
          </h4>
        ...
        {/* CTAs */}
        <div className="ns-cta-row flex flex-wrap items-center justify-center gap-4">
          <a
...
export default NovaSiteShowcase;
